...
const outputMessage = (msg) => {
    ...
    div.classList.add("p-2", "rounded", "w-fit", "m-2");
    ...
    if (!gameWithPlayersRows.game.is_active) {
        ...
        for (let i = 0; i < 6; i++) {
            if (i == 0 || i == 2) {
                ...
                empty_div(parent_div_list_of_players);
                continue;
            }
            if (i == 1) {
        ...
        // position of start button within last grid row
        for (let i = 0; i < 3; i++) {
            if (i == 0 || i == 2) {
                // const div = document.createElement("div");
                // parent_div_list_of_players.append(div);
...
const create_start_button = (gameId) => {
    ...
    button.addEventListener("click", async () => {
        const results = await axios.post(`/game/${getGameId()}/startGame`, {});
        console.log(results);
    });
    div.append(button);
    return div;
